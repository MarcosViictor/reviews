    ...
    return (
            ...
            <div >
                {searchResult.length > 0 ? (
                    <ul className='container-filmes-search container-filmes flex flex-wrap justify-center px-[10rem] phone:grid'>
                    {searchResult.map(movie => (
                        <Link to={`/overview-movie/${movie.id}`} key={movie.id}>
                            <li className="movie-card animate-fadeIn transition-all duration-50">
